...
To further investigate this peculiarity, a statistical inference method called chi-squared testing [@INSERT.CLASS.TEXTBOOK.REFERENCE.HERE] was used on the deviances of the models to assess if a variable can be removed without significantly affecting the model fit. To do this, the change in deviance measures the change between the log likelihood of the saturated model (in this case, the saturated model is the optimal model produced by the stepwise forward selection method) and the log likelihood of a reduced model. The reduced model is the same as the saturated model sans one variable. The deviance of each model was calculated for the purpose of showing the differences between a significant change vs. a non-significant change. The difference in deviance for each reduced model against the saturated model is calculated as:

$$
D = 2\phi(\ell_{saturated} − \ell_{reduced\ model})
$$

where $\phi$ is the dispersion parameter. This parameter is taken to be 1 for a binomial family. When deviance is scaled by the dispersion parameter $\phi$ , the scaled deviance follows a $\chi^2$ distribution. The test statistic for the $\chi^2$ distribution is then calculated as:

$$
\chi^2=D_{reduced}-D_{saturated}
$$

If removing a variable results in a large increase in deviance (and a small p-value less than 0.05 to show significance), it suggests that the variable plays an important role in the model fit. If this is not the case, the variable can be removed without significantly affecting the model fit.
...
<!--# VIF Citation --> [@salmerón2018a]

<!--# GLM Package Citation--> [@MASS]
