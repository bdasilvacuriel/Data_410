...
Recall earlier the estimates steps in the age function: (50-65, 65-75,75-100)
...
```{r Step.fwd, echo=FALSE, cache=TRUE}
# Intercept model
...
```{r Step.fwd.chisquared.tests, echo=FALSE, cache=TRUE}
...
kable(chi.results, digits = c(0, 0, 1, 2, 3), caption = "Deviance test results")


chi.results$P.value <- as.numeric(chi.results$P.value)
kable(chi.results, 
       caption = "Significance Tests for Reduced Models",
       digits = 8,
       label = "Chi-Squared Test Results",
       col.names = c("Variable Removed", "Deviance Change", "DF Difference", "P-value")) %>%
   kable_styling(bootstrap_options = c("striped", "condensed", "responsive"), 
                 full_width = FALSE) %>%
   column_spec(2, bold = TRUE, color = ifelse(chi.results$Deviance.Change == min(chi.results$Deviance.Change), "red","black")) %>%  # Make "Deviance Change" column bold
   column_spec(4, bold = TRUE, color = ifelse(chi.results$P.value > 0.05, "red", "black"))  # Highlight insignificant P-values
...
```{r Reduced.model.check, echo=FALSE, cache=TRUE}
## Fit reduced model
...
As there is some non-linear relationship present, a general additive model (GAM) is introduced to smooth this relationship to improve the models' fit. Since "Age" is the only variable in the model that is not categorical, we suspected that this variable may have a non-linear relationship with an Alzheimer's diagnosis. This smoothing is performed by using cubic splines on "Age" instead of the simple linear term. Recall in **[INSERT BENS SPLINE ANALYSIS RESULTS HERE]** where the number of splines used was four, allowed us to infer **[INSERT JUSTIFICATION FOR NUMBER OF SPLINES HERE]**.

```{r GAM.model, echo=FALSE, cache=TRUE}
alz_logit_gam<- gam(AlzheimerDiagnosis~ 
                   Country+ s(Age, k = 6)+ FamilyHistory+ GeneticRisk
                   ,knots = list(Age = c(65,65,75,75))
                  ,data = alzData
                  ,family = binomial)
...
By plotting the results of the gam model, a bimodal pattern can be seen, indicating that the residuals have a much stronger linear relationship infering that the GAM model eliminated the skewness from the GLM model.
...
```{r Log.odds, eval=FALSE, include=FALSE, cache=TRUE}
